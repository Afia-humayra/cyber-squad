...
TOPIC_CONFIG = {
    ...
    "multiplication": {
        ...
        "keywords": ["multiplication", "multiply", "times", "×", "*", "product", "multiplying", "multiplied"]
    },
    "division": {
        "file": "web-games/division.html",
        "audio": "audio_files/division.wav",
        "keywords": ["division", "divide", "÷", "/", "quotient", "dividing", "divided"]
    },
    "colours": {
...
def classify_topic_from_text(text):
    ...
    math_patterns = {
        ...
        "multiplication": [r'\*', r'×', r'multiply', r'times', r'product', r'\d+\s*[×*]\s*\d+'],
        "division": [r'÷', r'/', r'divide', r'quotient', r'\d+\s*[÷/]\s*\d+']
    }
...
def launch_file(filename, topic):
    ...
    try:
        if filename.endswith(".py"):
            ...
            try:
                subprocess.Popen(["chromium-browser", f"file://{os.path.abspath(filename)}"])
                logger.info(f"Opened {filename} in Chromium")
            except FileNotFoundError:
                try:
                    subprocess.Popen(["firefox", f"file://{os.path.abspath(filename)}"])
                    logger.info(f"Opened {filename} in Firefox")
                except FileNotFoundError:
...
def callback(recognizer, audio):
    ...
    try:
        ...
        if ("close" in text and "game" in text) or ("stop" in text and "game" in text):
            close_game()
            return
...
def main():
    """Main function."""
    print("Initializing Homi - Smart Study Assistant with Google Vision OCR")
    ...
    try:
        with microphone as source:
            ...
            print("Ready! Say 'Hello' to start or 'help me with homework' for OCR mode.")
    except Exception as e:
